...
func (s *scheduler) Schedule(ctx context.Context, req *ScheduleRequest) (*Task, error) {
	...
	if !s.running {
		s.runningMu.RUnlock()
		return nil, errors.InternalError("scheduler not running")
	}
	s.runningMu.RUnlock()
...
func (s *scheduler) Start() error {
	...
	if s.running {
		s.runningMu.Unlock()
		return errors.InternalError("scheduler already running")
	}
	s.running = true
	s.runningMu.Unlock()
...
func (s *scheduler) Stop() error {
	...
	if !s.running {
		s.runningMu.Unlock()
		return errors.InternalError("scheduler not running")
	}
	s.running = false
	s.runningMu.Unlock()
